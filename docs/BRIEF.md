...
- **Cuándo sale el correo: al despertarse, no a una hora fija.** Lo disparaba el cron de
  ...
  algo que tiene que pasar "al despertarte". Ahora hay varias fuentes y **`enviar_brief_si_toca()`
...
- **El interruptor** (`brief_ajustes`, `GET`/`PATCH /brief/ajustes`, panel ⚙ y las
  ...
  - **La comprobación va DENTRO de `enviar_brief_si_toca()` y solo ahí**, que es la única
    puerta del envío automático: puesta ahí las apaga todas de una vez, y una que se
    añada mañana no se puede olvidar de mirarla. Y va **antes de reservar**:

...
5. **Crons** (hecha) — el último run de cada workflow programado, los envíos del resumen
   y del informe, **de qué fuente salió cada resumen de los últimos 14 días**, las averías
   abiertas del vigilante y **quién sigue sondeando**.
...
| El resumen diario se mira por su FUENTE, no por si salió | El correo tiene cuatro disparadores y dos son relojes de respaldo, así que puede llegar todos los días y llegar mal todos los días: una señal de despertar muerta no da ningún error, el correo sale igual a las 10:00 en vez de cuando te levantas. El Atajo del iPhone ya se ha muerto en silencio dos veces (`docs/SALUD.md`). El panel `¿Salió al despertarte?` enseña los 14 últimos con su hora y su fuente, y el semáforo lo decide la **racha desde el último**: una mañana del reloj es el móvil descargado, tres seguidas son una integración caída. |
| Señal o reloj se distingue por `despertar_at`, no por el nombre de la fuente | Esa columna solo la escribe quien avisó de un despertar de verdad, así que un disparador nuevo entra en la cuenta sin que nadie tenga que acordarse de añadirlo a una lista — que es exactamente cómo el botón de la alarma pasó semanas sin mandar el correo. |
...
| Reconectar Outlook se puede pulsar **cuando todo funciona** | El otro botón de conectar vive en el widget del día detrás de `authNeeded`, o sea que solo aparece cuando la sesión YA está rota. El caso que hacía falta es el contrario: todo va bien y hay que pedir un permiso nuevo. Pasó al darle a Jarvis acceso al buzón (`Mail.ReadWrite` no estaba en el consentimiento viejo) y hubo que generar la URL a mano desde una sesión de Claude Code. Si el navegador bloquea la ventana emergente —`window.open` después de un `await` es justo lo que cazan—, la pantalla ofrece el enlace en vez de quedarse muda. |

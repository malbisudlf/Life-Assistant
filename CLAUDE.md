...
| `telefono/PARCHES.md` | **Antes de tocar o actualizar claude-phone.** Los ocho parches que necesita el repositorio original para funcionar aquí (credenciales SIP, modelo retirado, puertos, idioma, detector de voz, dominio del saliente, colgar si no lo coges, colgar si nadie habla). Viven solo en `caja` y **no están versionados**: un `claude-phone update` se los lleva |
